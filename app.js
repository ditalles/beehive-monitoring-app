    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
            <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-md">
                <h2 className="text-3xl font-bold text-center text-gray-800 mb-8">HoneyPi Login</h2>
                <div className="mb-5">
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8 font-inter">
            <header className="flex flex-col sm:flex-row justify-between items-center bg-white p-4 rounded-xl shadow-md mb-6">
                <h1 className="text-3xl font-extrabold text-gray-900 mb-2 sm:mb-0">HoneyPi Dashboard</h1>
...
const HiveDetail = ({ hive, onBack, userSettings, onCreateAlert }) => {
    ...
    }, [hive.id, selectedPeriod, userThingSpeakChannelId, userThingSpeakReadApiKey, userSettings.alertThresholds, onCreateAlert, hive.name]);
...
const App = () => {
        ...
        try {
            ...
            const unsubscribe = onAuthStateChanged(firebaseAuth, async (currentUser) => {
                if (currentUser) {
                    ...
                    await fetchUserSettings(firestore, currentUser.uid);
                    await fetchHives(firestore, currentUser.uid);
                    await fetchAlerts(firestore, currentUser.uid);
                } else {
                    try {
    ...
    const fetchUserSettings = async (firestore, currentUserId) => {
    ...
    const fetchHives = async (firestore, currentUserId) => {
        if (!firestore || !currentUserId) return;
        try {
            const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
    ...
    const fetchAlerts = async (firestore, currentUserId) => {
        if (!firestore || !currentUserId) return;
        try {
            const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
    ...
    const createAlert = useCallback(async (hiveId, hiveName, sensor, type, threshold, actual) => {
        ...
        try {
            ...
            console.log("Alert created:", { hiveId, sensor, type, threshold, actual });
        } catch (error) {
